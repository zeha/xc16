...
#if 0
  /* Don't inline functons that have varargs parameters, if the typelist doesn't
     end with a void - then its a varargs function */

  { tree last = tree_last (TYPE_ARG_TYPES (TREE_TYPE (fn)));

    if (last && TREE_VALUE (last) != void_type_node) {
      if (do_warning)
         warning ("%Jfunction %qF can never be inlined because it requires "
                 "a  variable argument list", fn, fn);
      goto cannot_inline;
    }
  }
#endif
