...
/* _PIC30_H_ the gty tool does not do well with macros inside the structure
   definitions.  Actually its a pretty weak tool for garbage collection and
   the thing should probably be scrapped - IMHO */
...
bool
set_named_section_flags (const char *section, SECTION_FLAGS_INT flags)
{
...
void
named_section_real (const char *name, SECTION_FLAGS_INT flags, tree decl)
{
...
{
#ifdef _PIC30_H_
  (*targetm.asm_out.select_section) (decl, 0, DECL_ALIGN (decl));
#else
  if (decl == NULL_TREE)
    text_section ();
  else
		   ...
		   int at_end ATTRIBUTE_UNUSED, int dont_output_data)
{
  ...
  if (DECL_INITIAL (decl) == error_mark_node)
    ...
    {
#ifdef _PIC30_H_
      pic30_push_constant_section(decl);
#endif
...
mark_constant (rtx *current_rtx, void *data)
{
  ...
  if (CONSTANT_POOL_ADDRESS_P (x))
      ...
      if (!TREE_ASM_WRITTEN (exp))
	{
          ...
          /* pic30_select_section no longer will select the section
             for constant strings */
          pic30_push_constant_section(0);
#endif
	  n_deferred_constants--;
	  output_constant_def_contents (x);
#ifdef _PIC30_H_
          pic30_pop_constant_section(0);
#endif
	}
    }
