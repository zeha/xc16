...
#define       PRE_ASM_SPEC    "%{!fsyntax-only:%{S:%W{o*}%{!o*:-o %b.s}}}"
#endif
...
"%{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\
 ...
 %{!fsyntax-only:%(pre_asm)}";

static const char *asm_options =
"%a %Y %{c:%W{o*}%{!o*:-o %w%b%O}}%{!c:%{save-temps:-o %w%b%O}%{!save-temps:-o %d%w%u%O}}";
	      ...
	      struct prefix_list *pl = include_prefixes.plist;
              ...
              if (*p == '-') {
                 ...
                 while (strncmp(p,"../", 3) == 0) {
                   ...
                   while (*c == 0) c--;
                   while (IS_DIR_SEPARATOR(*c) && (c > prefix)) c--;
                   while (!IS_DIR_SEPARATOR(*c) && (c > prefix)) c--;
                   c[1] = 0;
                   p+=3;
                 }
...
#define SKIP_WHITE() do { while (*p == ' ' || *p == '\t') p++; } while (0)
 ...
 invalid:
  fatal ("braced spec '%s' is invalid at '%s'", orig, p);
