  ...
  if (osize < UNITS_PER_WORD)
    {
      ...
      static unsigned int low_off;
      low_off = subreg_lowpart_offset (omode, wmode);
		  ...
		  int validate, int adjust)
{
  ...
  if (adjust)
	...
	addr = gen_rtx_LO_SUM (GET_MODE(XEXP(addr,0)), XEXP (addr, 0),
...
offset_address (rtx memref, rtx offset, unsigned HOST_WIDE_INT pow2)
{
  ...
  gcc_assert(GET_CODE(memref) == MEM);
  pointer_mode = TARGET_IS_POINTER_MODE(GET_MODE(XEXP(memref,0))) ?
                                    GET_MODE(XEXP(memref,0)) : Pmode;
                                                          
  new = simplify_gen_binary (PLUS, pointer_mode, addr, offset);
