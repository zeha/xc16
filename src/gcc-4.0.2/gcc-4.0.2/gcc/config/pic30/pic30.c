...
static const char *default_section_name(tree decl, SECTION_FLAGS_INT flags) {
  ...
  *f = 0;
  ...
  if (decl) {
    ...
    if (u)  f+= sprintf(result,"%s", pic30_default_section); 
    else if (a) {
      if (!pszSectionName||(strcmp(pszSectionName,pic30_default_section) == 0))
        f += sprintf(result, "%s_0x%lx,%s(%ld)",
      ...
      is_default = 1;
      f+= sprintf(result,"%s", pic30_default_section);
    }
    if (b) {
    ...
    if ((!is_default) || (f > &result[1])) return result;
  }
  if (flags) {
    i = 0;
...
pic30_assemble_integer(rtx x, unsigned int size, int aligned_p)
{
  ...
  /* First try to use the standard 1, 2, 4, 8, and 16 byte
    ...
    if ((GET_MODE(x) == P24PSVmode) || (GET_MODE(x) == P24PROGmode)) {
      ...
      {
#endif
        fprintf(asm_out_file, "\t.long psvptr(");
        output_addr_const(asm_out_file, x);
        fprintf(asm_out_file, ")\n");
        return 1;
      }
    ...
    */
    ...
    case PIC30_BUILTIN_ADD: {
      ...
      if (pic30_errata_mask & psv_errata) {
        emit_insn(
          (INTVAL(r1) < 0) ? gen_addacr_shiftlt_hi_errata(target, r0, r1) :
...
int pic30_near_operand(rtx op, enum machine_mode mode) {
    ...
    fNear = ((mode == GET_MODE(op)) && pic30_U_constraint(op));
...
int pic30_emit_move_sequence(rtx *operands, enum machine_mode mode) {
  ...
  if (!reload_in_progress) {
    ...
    if (((mode == P24PROGmode) || (mode == P24PSVmode)) &&
         pic30_symbolic_address_operand(op1,mode)) {
      rtx (*fn)(rtx,rtx) = 0;
  ...
  */
  ...
  if (pic30_accumulator_operand(op0,mode)) {
    ...
    if (pic30_T_constraint(op1) || pic30_U_constraint(op1)) {
  ...
  if (pic30_accumulator_operand(op1,mode)) {
    ...
    if (pic30_T_constraint(op0) || pic30_U_constraint(op0)) {
  ...
  if (!noreturn) {
    ...
    */
    ...
    if (is_boot_fn) {
      ...
      emit_insn(gen_movqi_address(reg,sfr));
      emit_insn(gen_set_psv(reg));
...
pic30_expand_epilogue(void)
{
  ...
  if ((is_boot_fn || is_secure_fn) && (psv_model != psv_unspecified)) {
    psv_model = psv_unspecified;
  } else if ((fInterrupt) && (psv_model == psv_unspecified)) {
    psv_model = psv_auto_psv;
  }
...
*/
int
pic30_valid_machine_decl_interrupt_attribute(args, attached_to, decl)
     tree args;
     char *attached_to;
     tree decl;
{
...
{  sectionStack *s;
   ...
   if (lSectionStack->flags & ss_should_pop) {
     ...
     s = freeSectionStack;
     if (s->pszName) free(s->pszName);  /* avoid leak */
     s->pszName = 0;
    ...
    rtx value;
{
  ...
  if ((GET_MODE(value) == P24PSVmode) || (GET_MODE(value) == P24PROGmode)) {
    operator="psvptr(";
  } else  if (sym = pic30_program_space_operand_p(value)) {
...
rtx pic30_legitimize_address(rtx x, rtx oldx ATTRIBUTE_UNUSED,
  ...
  switch (GET_CODE(x)) {
    default: break;
    case MEM: if (GET_MODE(x) == P24PROGmode) {
                return x;
              } else if (GET_MODE(x) == P24PSVmode) {
                return x;
              }
  }
  return 0;
          ...
          case P24PROGmode:
             emit_insn(
               gen_movP24PROG_address(to,
                                   gen_rtx_SYMBOL_REF(P24PROGmode,XEXP(from,0)))
             );
             return 1;
          case P24PSVmode:
             emit_insn(
               gen_movP24PSV_address(to,
                                    gen_rtx_SYMBOL_REF(P24PSVmode,XEXP(from,0)))
             );
             return 1;
          default: break;
        }
  ...
  if ((newmode == P24PROGmode) || (newmode == P24PSVmode)) {
    switch (oldmode) {
      ...
      case SImode: {
        ...
        emit_insn(gen_rotlhi3(gen_rtx_SUBREG(HImode,result,0),
...
*/
    ...
    switch (newmode) {
      ...
      case HImode: {
        ...
        result = gen_reg_rtx(newmode);
        emit_move_insn(result, gen_rtx_SUBREG(newmode, temp, 0));
        emit_insn(gen_rotrhi3(result,result));
        break;
      }
      case SImode: {
        rtx temp;
 ...
 */
enum gimplify_status pic30_can_gimplify_expr(tree *expr_p) {
  tree expr = *expr_p;
