...
find_base_value (rtx src)
{
  ...
  switch (GET_CODE (src))
    ...
    case TRUNCATE:
      /*
       *  Perhaps should be:  < GET_MODE_SIZE(GET_MODE(XEXP(src,0)))  (CW)
       */
		  ...
		  enum machine_mode y_mode)
{
  ...
  if ((GET_CODE (x_base) == ADDRESS && 
       TARGET_IS_POINTER_MODE(GET_MODE(x_base))) || 
      (GET_CODE (y_base) == ADDRESS && 
       TARGET_IS_POINTER_MODE(GET_MODE(y_base))))
    return 0;
  ...
  switch (GET_CODE (x))
      ...
      if (base)
	{
	  ...
	  if (GET_CODE (base) == ADDRESS
	      && TARGET_IS_POINTER_MODE(GET_MODE (base))
