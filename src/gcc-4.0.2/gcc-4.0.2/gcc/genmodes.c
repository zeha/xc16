		    ...
		    const char *file, unsigned int line)
{
  ...
  for (m = modes[cl]; m; m = m->next)
	...
	continue;
      ...
      /* is target pointer modes are special INTS used by targets to define
         new pointer widths, and are not suitable for complex types */
...
make_int_mode (const char *name,
	       unsigned int precision, unsigned int bytesize,
               unsigned int is_target_pointer, const char *file, 
               unsigned int line)
{
...
#define PARTIAL_INT_MODE2(M,N) \
  make_partial_integer_mode (#M, #N, -1U, __FILE__, __LINE__)
