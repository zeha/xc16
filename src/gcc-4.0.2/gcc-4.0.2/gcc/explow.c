...
/* Some targets define more than one pointer mode, use the TARGET function to
   determine what the correct pointer mode for mode is */
...
memory_address (enum machine_mode mode, rtx x)
{
  ...
  x = convert_memory_address (pointer_mode, x);
  ...
  /* We get better cse by rejecting indirect addressing at this stage.
	...
	/* x = force_reg (Pmode, x); */
        /* we are trying to make a thing of 'mode' which may or may not be
             Pmode */
        x = force_reg (pointer_mode, x);
...
force_reg (enum machine_mode mode, rtx x)
{
  ...
  if (general_operand (x, mode))
	...
	{
	  rtx temp2 = gen_reg_rtx (GET_MODE(temp));
