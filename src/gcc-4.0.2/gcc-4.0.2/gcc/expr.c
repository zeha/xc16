...
emit_single_push_insn (enum machine_mode mode, rtx x, tree type)
{
  ...
  /* If we are to pad downward, adjust the stack pointer first and
  ...
  else
    {
...
#else
      /* must ensure that when the data is written to the stack; the stack
         pointer is already in the correct place for a new store - ie if
         an interrupt occurs after the store; interrupt processing will not
         disturb the stored data. */
      /* with a decreasing stack pointer, this can be done without worry
         generate the new stack pointer, then issue the store via the stack
         pointer */
      emit_move_insn(stack_pointer_rtx, dest_addr);
      ...
      dest_addr = gen_rtx_POST_MODIFY (Pmode, stack_pointer_rtx, dest_addr);
#else
      /* must ensure that when the data is written to the stack; the stack
         pointer is already in the correct place for a new store - ie if
         an interrupt occurs after the store; interrupt processing will not
         disturb the stored data. */
      /* with an increasing stack pointer, this is more difficult: increment
         the stack pointer then store at sp - increment.  In this way an
         interrupt will never overwrite the value about to be stored */
      emit_move_insn(stack_pointer_rtx, dest_addr);
      dest_addr = gen_rtx_PLUS(Pmode, stack_pointer_rtx,
                               GEN_INT (-(HOST_WIDE_INT) rounded_size));
#endif
...
#define REDUCE_BIT_FIELD(expr)	(reduce_bit_field && !ignore		  \
  ...
  /* IF the reg is a USERVAR; then the user probably wants to use that register
     for some reason... this is certainly true for DSPIC (CW) */
 
  if (! cse_not_expected && mode != BLKmode && target
        && (GET_CODE (target) != REG ||
            ((REG_USERVAR_P(target) == 0) &&
              (REGNO(target) < FIRST_PSEUDO_REGISTER)))
  ...
  switch (code)
	...
	temp = validize_mem (DECL_RTL (exp));
	 ...
	 the address is not valid (for direct access) or it is not a register 
         and -fforce-addr is specified, get the address into a register.  */
	...
	if (offset != 0)
	  {
		...
		TREE_INT_CST_LOW(TREE_OPERAND (exp, 0)) == 2) ||
	    ...
	    gcc_assert (MEM_P (op0));
            ...
            /* op0 is a valid mem for the form:   
                    (mem:BLK/Target mode ( ptr: valid_ptr_mode))
               The offset must match the mode of valid_ptr_mode */
#ifdef POINTERS_EXTEND_UNSIGNED
	    if (GET_MODE (offset_rtx) != GET_MODE(XEXP(op0,0)))
	      offset_rtx = convert_to_mode (GET_MODE(XEXP(op0,0)), 
                                            offset_rtx, 0);
#else
	    if (GET_MODE (offset_rtx) !=  GET_MODE(XEXP(op0,0)))
	      offset_rtx = convert_to_mode (GET_MODE(XEXP(op0,0)), 
	    ...
	    rtx table_label ATTRIBUTE_UNUSED, rtx default_label)
{
  ...
  result = gen_casesi (index, op1, op2, table_label, default_label);
