...
documented_lang_options[] =
{
  ...
  { "-fpreprocessed", "" },
  { "-fno-preprocessed", "" },
#if !defined(TARGET_SHORT_DOUBLE) || (TARGET_SHORT_DOUBLE == 0)
  { "-fshort-double",
    N_("Use the same size for double as for float") },
  { "-fno-short-double", "" },
#else
  { "-fshort-double", "" },
  { "-fno-short-double", N_("Use a double that is larger than float") },
#endif
  { "-fshort-enums",
     ...
     tree decl;
{
  ...
  if (optimize > 0)
      ...
      if (rebuild_jump_labels_after_combine)
	{
      ...
      simple_combine(insns, max_reg_num());
  ...
  if (optimize > 0 && flag_peephole2)
    { int i;
      ...
      for (i = 0; i  < 4; i++) {
        if (peephole2_optimize (rtl_dump_file) == 0) break;
      }
