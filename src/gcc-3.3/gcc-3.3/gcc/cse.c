     ...
     rtx libcall_insn;
{
  ...
  static int in_libcall = 0;
  static int prev_in_libcall = 0;
  ...
  if (find_reg_note (insn, REG_LIBCALL, NULL_RTX)) in_libcall = 1;
	  ...
	  && ! find_reg_note (insn, REG_RETVAL, NULL_RTX))
	{
	  ...
	  if (prev != 0 && GET_CODE (prev) == INSN
...
#else
              /* set the current instructions src to the src of the previous
                 instruction */
              if ((SET_SRC(PATTERN(prev)) == SET_SRC(sets[0].rtl)) &&
                  !prev_in_libcall) {
                  /* SET (X) (X);  SET (Y) (X) -> 
                     SET (Y) (X);  delete  */
		...
		remove_note (insn, note);
              ...
              if (note) {
                 /* if the previous instruction has a reg_equal, add it
                    to this instruction */
                 note = copy_rtx(note);
  ...
  prev_insn = insn;
  prev_in_libcall = in_libcall;
  if (in_libcall && find_reg_note (insn, REG_RETVAL, NULL_RTX)) in_libcall=0;
