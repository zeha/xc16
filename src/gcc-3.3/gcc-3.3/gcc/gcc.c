...
static       char *pic30_resource_file;
#endif
...
static const char path_separator_str[] = { PATH_SEPARATOR, 0 };
...
/*   May also define DEFAULT_LIB_PATH to set LIBRARY_PATH_ENV if this variable
     is not set */
#ifndef LIBRARY_PATH_ENV
#define LIBRARY_PATH_ENV "LIBRARY_PATH"
#else
#define NON_DEFAULT_LIBRARY_PATH 1
#endif
...
or with constant text in a single argument.
 ...
 %I-	Substitute GCC_EXEC_PREFIX, if a relative path immediately follows, then
        adjust the path if ../ is part of the pathname:  Ie %I-../ would omit
...
%{!fsyntax-only:%{!c:%{!M:%{!MM:%{!E:%{!S:\
    %(program_prefix)%(linker) %l %X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} %{r} %{s} %{t}\
...
static const char *pre_asm_spec = PRE_ASM_SPEC;
...
"%a %Y %{c: %{o*}%{!o*:-o %w%b%O}}%{!c:%{save-temps:-o %w%b%O}%{!save-temps:-o %d%w%u%O}}";

static const char *invoke_as =
"%{!S:%(program_prefix)as %(asm_options) %{!pipe:%{save-temps: %b.s} %{!save-temps: %g.s}} %A }";
...
%eGNU C no longer supports -traditional without -E}\
	  %{save-temps|traditional-cpp|no-integrated-cpp:%(trad_capable_cpp) \
		%(cpp_options) %{save-temps:%b.i} %{!save-temps:%g.i} \n\
		    %(program_prefix)cc1 -fpreprocessed %{save-temps:%b.i} %{!save-temps:%g.i} %(cc1_options)}\
  ...
  {".i", "@cpp-output", 0},
  {"@cpp-output",
   "%{!M:%{!MM:%{!E:%(program_prefix)cc1 -fpreprocessed %i %(cc1_options) %{!fsyntax-only:%(invoke_as)}}}}", 0},
  {".s", "@assembler", 0},
  {"@assembler",
   "%{!M:%{!MM:%{!E:%{!S:%(program_prefix)as %(asm_debug) %(asm_options) %i %A }}}}", 0},
  {".S", "@assembler-with-cpp", 0},
  {"@assembler-with-cpp",
   "%(trad_capable_cpp) -lang-asm %(cpp_options)\
      %{E|M|MM:%(cpp_debug_options)}\
      %{!M:%{!MM:%{!E:%{!S:-o %{|!pipe:%g.s} |\n\
       %(program_prefix)as %(asm_debug) %(asm_options) %{!pipe:%g.s} %A }}}}", 0},
#include "specs.h"
  /* Mark end of table */
  {0, 0, 0}
};
...
static struct spec_list static_specs[] =
{
  ...
  INIT_STATIC_SPEC ("pre_asm",                  &pre_asm_spec),
     ...
     const char *const *argv;
{
  ...
  if (!gcc_exec_prefix)
    {
      ...
      if (gcc_exec_prefix)
	putenv (concat (GCC_EXEC_PREFIX_ENV "=", gcc_exec_prefix, NULL));
    }
#endif
  ...
  GET_ENVIRONMENT (temp, COMPILER_PATH_ENV);
  if (temp)
    {
  ...
  {
    const char *library_paths[] = { LIBRARY_PATH_ENV, "LPATH", -1 };
    int pathNo = 0;
  ...
  for (i = 1; i < argc; i++)
	...
	{ 
	  /* translate_options () has turned --version into -fversion.  */
                ...
                if (Microchip) {
                  ...
                  if (*Microchip) {
                    ...
                    *Microchip = 0;
                    ...
                    mismatch = (vid != (rib->version.major*100 + 
                                        rib->version.minor));
                    if (mismatch) {
	    ...
	    printf (_("__C30_VERSION__ == %d\n"), vid);
          }
#else
	  printf (_("%s (GCC) %s\n"), programname, version_string);
#endif
	  fputs (_("Copyright (c) 2004 Free Software Foundation, Inc."
					" All rights reserved.\n"), stdout);
	  ...
	  case 'I':
	    {
	      struct prefix_list *pl = include_prefixes.plist;
              ...
              if (*p == '-') {
                 /* just print out gcc_exec_prefix, adjusting if needed */
                 char *prefix = xstrdup(gcc_exec_prefix);
                 ...
                 while (strncmp(p,"../", 3) == 0) {
                   ...
                   while (*c == 0) c--;
                   while ((*c == DIR_SEPARATOR) && (c > prefix)) c--;
                   while ((*c != DIR_SEPARATOR) && (c > prefix)) c--;
                   c[1] = 0;
                   p+=3;
                 }
                 do_spec_1(prefix,1,NULL);
                 free(prefix);
                 break;
              }
	  ...
	  case 'p':
	    {
              if (*p == 'a') {
                p++;
                value = do_spec_1(pre_asm_spec, 0, NULL);
...
next_member:
 ...
 next_ampersand:
  filter = p;
  /* star_found is true iff:  \*[ ]*[:}|&] - if you see what I mean */
  while (*p != ':' && *p != '}' && *p != '|' && *p != '&') {
    if (*p != ' ') star_found = 0;
    if (*p == '*') star_found = 1;
    p++;
  ...
  if (suffix)
  ...
  else if (star_found && (p[0] == '}' || p[0] == '&'))
    {
      ...
      pp++;
      for (i = 0; i < n_switches; i++)
	if (!strncmp (switches[i].part1, filter, pp - 1 - filter)
	    && check_live_switch (i, pp - 1 - filter))
	  {
	    if (elide_switch)
	      {
		...
		if (!strncmp (switches[i].part1, filter, hard_match_len)
		    && check_live_switch (i, -1))
		  {
		    if (do_spec_1 (string, 0, 
                                   &switches[i].part1[hard_match_len]) == -1) 
                      return 0;
      ...
      else if (pipe_p)
	{
	  ...
	  if (do_spec_1 ("-", 0, NULL) == -1) return 0;
	  return endbody;
	}
    }
     ...
     const char *const *argv;
{
  ...
  for (i = 0; (int) i < n_infiles; i++)
      ...
      if (input_file_compiler)
	{
	  ...
	  if (input_file_compiler->spec[0] == '#')
	    {
#ifdef _PIC30_H_
              if (input_file_compiler->spec[1] != 'c')
                 error ("%s: %s compiler not supported on this system",
  ...
  if (error_count == 0)
      ...
      if (! strcmp (linker_name_spec, "collect2"))
	{
      ...
      /* Rebuild the COMPILER_PATH and LIBRARY_PATH environment variables
	 for collect.  */
      putenv_from_prefixes (&exec_prefixes, COMPILER_PATH_ENV);
