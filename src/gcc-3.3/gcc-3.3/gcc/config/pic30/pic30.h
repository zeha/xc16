...
extern char *pic30_default_include_path() __attribute__((weak));
...
*/
#define TARGET_SMALL_AGG   ((target_flags & TARGET_MASK_LARGE_AGG) == 0)

/*
** Small scalar data model means that scalar objects can be
** addressed using the short 13-bit direct address.
*/
#define TARGET_SMALL_SCALAR   ((target_flags & TARGET_MASK_LARGE_SCALAR) == 0)

/*
** Large data model means that data objects must be
** addressed using the long 16-bit direct address.
*/
#define TARGET_LARGE_AGG   ((target_flags & TARGET_MASK_LARGE_AGG) != 0)

/*
** Large scalar model means that scalar objects must be
** addressed using the long 16-bit direct address.
*/
#define TARGET_LARGE_SCALAR   ((target_flags & TARGET_MASK_LARGE_SCALAR) != 0)

/*
** Small code model means that calls to functions
** can be done using the short RCALL instruction.
*/
#define TARGET_SMALL_CODE   ((target_flags & TARGET_MASK_LARGE_CODE) == 0)
...
** stage is run.
*/
#define TARGET_PA             ((target_flags & TARGET_MASK_PA) != 0)

/*
** No procedure aggregation means that the procedure aggregation optimization
** stage is not run.
*/
#define TARGET_NO_PA          ((target_flags & TARGET_MASK_PA) == 0)
#endif
 ...
 */
#define TARGET_ARCH(X)  ((target_flags & TARGET_MASK_ARCH_ ## X) != 0)

/*
 *  TARGET_SMART_IO evaluates to 0,1,2
 *    0 if TARGET_MASK_SMART_IO not set
 *    1 if TARGET_MASK_SMART_IO set and TARGET_MASK_SMART_IO_LVL clear
 *    2 if TARGET_MASK_SMART_IO set and TARGET_MASK_SMART_IO_LVL set
 */
#define TARGET_SMART_IO ((target_flags&TARGET_MASK_SMART_IO) ? (pic30_io_size_val) : 0)
...
#define TARGET_SWITCHES                    \
{                           \
 {"small-data",-TARGET_MASK_LARGE_DATA, "Use small data model"},   \
 {"large-data", TARGET_MASK_LARGE_DATA, "Use large data model"},   \
 {"small-scalar",-TARGET_MASK_LARGE_SCALAR, "Use small scalar data model"}, \
 {"large-scalar", TARGET_MASK_LARGE_SCALAR, "Use large scalar data model"}, \
 {"small-code",-TARGET_MASK_LARGE_CODE, "Use small code model"},   \
 {"large-code", TARGET_MASK_LARGE_CODE, "Use large code model"},   \
...
( \
   (C) == 'I' ? (VALUE) == 1 :                                      \
   (C) == 'J' ? 0 <= (VALUE) && (VALUE) <= 1023 :                   \
   (C) == 'K' ? 0 <= (VALUE) && (VALUE) <= 15 :                     \
   (C) == 'L' ? 0 != (VALUE) && (VALUE) <= 2 && (VALUE) >= -2:      \
   (C) == 'M' ? -1023 <= (VALUE) && (VALUE) < 0 :                   \
   (C) == 'N' ? -31 <= (VALUE) && (VALUE) < 0 :                     \
   (C) == 'O' ? (VALUE) == 0 :                                      \
   (C) == 'P' ? 0 <= (VALUE) && (VALUE) <= 31 :                     \
   (C) == 'W' ? ((-16 <= VALUE) && (VALUE <= 16)) :                 \
   (C) == 'Y' ? ((VALUE == -6) || (VALUE == -4) || (VALUE == -2) || \
                 (VALUE == 2) || (VALUE == 4) ||                    \
                 (VALUE == 6)) :                                    \
   (C) == 'Z' ? -8 <= (VALUE) && (VALUE) <= 7 :                     \
    0)
...
#define FUNCTION_VALUE(VALTYPE, FUNC)  \
  gen_rtx(REG,TYPE_MODE(VALTYPE),WR0_REGNO)
...
#define LIBCALL_VALUE(MODE)  \
   gen_rtx(REG, MODE, WR0_REGNO)   /* Return in W0 */
...
#define REGISTER_TARGET_PRAGMAS(PFILE) { \
  cpp_register_pragma(PFILE, 0, "interrupt", pic30_handle_interrupt_pragma); \
  cpp_register_pragma(PFILE, 0, "code", pic30_handle_code_pragma); \
  cpp_register_pragma(PFILE, 0, "idata", pic30_handle_idata_pragma); \
  cpp_register_pragma(PFILE, 0, "udata", pic30_handle_udata_pragma); }
...
*/
#define CONST_SECTION_ASM_OP pic30_const_section_asm_op()
...
#define GCC_DRIVER_HOST_INITIALIZATION { \
  ...
  pic30_resource_file = xmalloc(size);                                \
                                                                      \
  sprintf(pic30_resource_file, "%s",argv[0]);                         \
  pic30_resource_file[p-argv[0]] = 0;                                 \
