...
static int validate_insn(rtx insn);

...
void simple_combine(rtx f, unsigned int nregs) {
  rtx insn;
  int i;
      ...
      for (insn = this_basic_block->head;
           ...
           insn = NEXT_INSN (insn))
        {
           if ((GET_CODE(insn) == INSN) &&
               ...
               (GET_CODE(XEXP(PATTERN(insn),1)) == COMPARE)) {
             int insn_code_comp, insn_code_jump = 0;
             rtx new_insn;
             rtx temp,notes;
             enum rtx_code old_code = GET_CODE(PATTERN(insn));

             subst_insn = insn;
             /* this is normally cleared by try_combine() but we don't call it*/
             undobuf.other_insn = 0;
             new_insn = combine_simplify_rtx(PATTERN(insn),VOIDmode,1,0);
             if (new_insn && GET_CODE(new_insn) != CLOBBER) {
               insn_code_comp = recog_for_combine(&new_insn,insn,&notes);
               if (insn_code_comp < 0) {
                 undo_all();
               } else {
                 if (undobuf.other_insn)
                   insn_code_jump = validate_insn(undobuf.other_insn);
                 if (insn_code_jump < 0) undo_all();
                 else {
                   if (notes) {
                     for (temp = notes; temp; temp = XEXP (temp, 1))
                       if (GET_CODE (XEXP (temp, 0)) == REG)
                         REG_N_DEATHS (REGNO (XEXP (temp, 0)))++;
                     distribute_notes(notes, insn, insn, NULL_RTX,
                                      NULL_RTX, NULL_RTX);
                   }
                   INSN_CODE(insn) = insn_code_comp;
                   if (undobuf.other_insn)
                     INSN_CODE(undobuf.other_insn) = insn_code_jump;
                   undo_commit();
...
#if defined(CANONICALIZE_COMPARISON)
          /* we might never get a chance to do CANONICALIZE_COMPARISON */
                   ...
                   subst_insn = insn;
                   /* this is normally cleared by try_combine() but we don't 
                      call it*/
                   undobuf.other_insn = 0;
                   new_insn = combine_simplify_rtx(PATTERN(insn),VOIDmode,1,0);
                   if (new_insn && GET_CODE(new_insn) != CLOBBER) {
                      ...
                      if (insn_code_comp < 0) {
                        ...
                        if (undobuf.other_insn) 
                          insn_code_jump = validate_insn(undobuf.other_insn);
                        if (insn_code_jump < 0) undo_all();
                        /* simplify gotten rid of test? removes an edge from
                           bb without removing edge */
                        else if (undobuf.other_insn &&
                            GET_CODE(XEXP(PATTERN(undobuf.other_insn),1)) !=
                            IF_THEN_ELSE) undo_all();
                        else {
                          if (notes) {
                            for (temp = notes; temp; temp = XEXP (temp, 1))
                              if (GET_CODE (XEXP (temp, 0)) == REG)
...
static int validate_insn(rtx insn) {
  ...
  if (insn) {
      int other_code_number;
      rtx other_pat = PATTERN (insn);
      rtx new_other_notes;
      rtx note, next;
...
#else
  /* since there is no guarantee that i1->i2->i3 with no instructions between
     i2 and i3 (or i1 and i2, for that matter).  This is not a safe way to
     determine if i1 and i2 are not needed.  
  
     Scan forward from i1, looking for a use of i1 or i2 before i3 instead */
  { rtx scan;
    int i2_seen = 0;

    added_sets_1 = added_sets_2 = 0;
    for (scan = i1 ? i1 : i2; scan != i3; scan = NEXT_INSN(scan))
    {
      if (INSN_P(scan))
      {
        /* if scan == i2, this is checked at the end */
        if (i1 && (scan != i2) && (scan != i1) && 
            reg_overlap_mentioned_p(i1dest, scan)) 
	 if (i1_feeds_i3)
          added_sets_1 = 1;
        if (i2_seen && reg_overlap_mentioned_p(i2dest, scan))
          added_sets_2 = 1;
      }
      if (scan == i2)
          i2_seen = 1;
    ...
    added_sets_1 |= (i1 && 
                     !(i1_feeds_i3 ? dead_or_set_p (i3, i1dest)
                                   : (dead_or_set_p (i3, i1dest) || 
...
#else
  other_code_number = validate_insn(undobuf.other_insn);
#endif
