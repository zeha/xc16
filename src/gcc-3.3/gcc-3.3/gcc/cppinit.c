...
/* define a default value for C_INCLUDE_PATH to be used if it is not defined.
     ...
     int path;
{
  ...
  if ((p == NULL)  || (*p == 0))
    {
      if (executable_path_name == 0) {
        int i;
   
        executable_path_name = xstrdup(save_argv[0]);
        for (i = strlen(executable_path_name); i; i--)
          if (executable_path_name[i] == DIR_SEPARATOR)
          {
             executable_path_name[i+1] = 0;
             break;
          }
      }
      p = (char *) DEFAULT_INCLUDE_PATH;
      insert = (char *) executable_path_name;
      insert_length = strlen(executable_path_name);
      if ((executable_path_name == 0) || (*executable_path_name == 0)) return;
    }
#endif
  ...
  do
      ...
      else
	{
	  ...
	  name = (char *) xmalloc (q - p + 1 + insert_length);
          if (insert) memcpy(name, insert, insert_length);
	  memcpy (name + insert_length, p, q - p);
	  name[insert_length + q - p] = 0;
	}
     ...
     cpp_reader *pfile;
{
  ...
  switch ((CPP_OPTION (pfile, objc) << 1) + CPP_OPTION (pfile, cplusplus))
    {
    case 0:
      GET_ENVIRONMENT (path, C_INCLUDE_PATH_ENV);
      break;
    case 1:
  ...
  /* if DEFAULT_INCLUDE_PATH is defined, path_include will define that path
     in the even of path being NULL */
  if (path != 0 && *path != 0)
#endif
